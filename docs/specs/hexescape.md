...
**Status:** Refined v7 — balance & level pass (round-4)
**Date:** 2026-06-16
**Authors:** analyst + architect
**Revision:** 7 — round-4 playtest balance & level pass. Applies 5 decisions (D1–D5): tutorial-01 level rework — sparser non-Cross pre-placed tiles, Straight horde-origin tiles, ≥2 exit approach cells, route-blocking starting zombies (D1); difficulty/pacing constant changes — SafeOpeningFraction 0.30→0.20, StartingHandSize 2→1, ApPoolSize [5,4,4,3,3,3]→[5,4,4,4,4,4], ExitBandFraction mid-count raise, ZombieTileMinSpacing=2 zombie-tile gap deck construction (D2); draw-fast mitigation design note added as AD-OB-12b (D3); MF-1/AC-v2-8b forced zombie-tile placement prefers character-free cells (D4); zombie/exit-zone movement invariant clarified in AC-v2-32d and AD-OB-7/MF-3 (D5). AC count: 82 (up from 79; three new sub-criteria as testable assertions: AC-v2-5 ≥2 exit-approach cells, AC-v2-1b zombie-spacing clause, AC-v2-8b character-free preference). AD count: 14 numbered ADs plus AD-OB-12b design note (AD-OB-7, AD-OB-12, AD-OB-13 updated in place; AD-OB-12b added). Band math verified valid for all 6 player counts with new constants (SafeOpeningFraction=0.20, StartingHandSize=1, updated ExitBandFraction) — all constraints satisfied.
...
**Band math worked examples (D2 — verified with StartingHandSize=1, SafeOpeningFraction=0.20, updated ExitBandFraction):**

For each player count n, verify: (a) safeRemaining ≥ 0, (b) middleBand ≥ zombieTileCount, (c) exitBand ≥ 1, (d) safeRemaining + middleBand + exitBand = postDealSize, (e) no negative or overlapping bands.

| Players | postDealSize | rawPoolSize = postDealSize + (1×n) | safeCount = floor(rawPool × 0.20) | safeRemaining = safeCount − (1×n) | exitBandFraction | exitBandStart = postDealSize − floor(postDealSize × frac) | exitBand = postDealSize − exitBandStart | middleBand = exitBandStart − safeRemaining | zombieTiles | safeRemaining ≥ 0? | middleBand ≥ zombieTiles? |
|---------|-------------|--------------------------------------|-----------------------------------|-----------------------------------|-----------------|-----------------------------------------------------------|----------------------------------------|--------------------------------------------|-------------|-------------------|--------------------------|
| 1       | 30          | 31                                   | floor(31×0.20)=6                  | 6−1=5                             | 0.50            | 30−floor(30×0.50)=30−15=15                               | 15                                     | 15−5=10                                    | 3           | 5 ≥ 0 ✓           | 10 ≥ 3 ✓                 |
| 2       | 40          | 42                                   | floor(42×0.20)=8                  | 8−2=6                             | 0.40            | 40−floor(40×0.40)=40−16=24                               | 16                                     | 24−6=18                                    | 5           | 6 ≥ 0 ✓           | 18 ≥ 5 ✓                 |
| 3       | 50          | 53                                   | floor(53×0.20)=10                 | 10−3=7                            | 0.40            | 50−floor(50×0.40)=50−20=30                               | 20                                     | 30−7=23                                    | 7           | 7 ≥ 0 ✓           | 23 ≥ 7 ✓                 |
| 4       | 60          | 64                                   | floor(64×0.20)=12                 | 12−4=8                            | 0.35            | 60−floor(60×0.35)=60−21=39                               | 21                                     | 39−8=31                                    | 10          | 8 ≥ 0 ✓           | 31 ≥ 10 ✓                |
| 5       | 70          | 75                                   | floor(75×0.20)=15                 | 15−5=10                           | 0.32            | 70−floor(70×0.32)=70−22=48                               | 22                                     | 48−10=38                                   | 12          | 10 ≥ 0 ✓          | 38 ≥ 12 ✓                |
| 6       | 80          | 86                                   | floor(86×0.20)=17                 | 17−6=11                           | 0.30            | 80−floor(80×0.30)=80−24=56                               | 24                                     | 56−11=45                                   | 15          | 11 ≥ 0 ✓          | 45 ≥ 15 ✓                |

All 6 player counts pass all four constraints. Band math is valid for all counts with the new constants. Middle bands are comfortably larger than zombie tile counts, leaving room for `ZombieTileMinSpacing=2` gap enforcement in all counts.

**Balance notes (D2):** `ApPoolSize[1] = 5` is higher than for 2–6 players (all now 4) — solo gets 1 extra AP. `ExitBandFraction` raised for 2–6p so the exit surfaces earlier across all multi-player counts; the critical fix is 3p (0.30→0.40), which was unwinnable because teams were overrun before the exit appeared. All values are balance TBD by playtest.

### AD-OB-12b: Design note — draw-fast mitigation (D3)

DrawTile still counts as a qualifying action (no rule change). The draw-fast-to-surface-the-exit incentive is a known dominant strategy in early play: players draw as fast as possible to reveal the exit early, then race to it. This incentive is countered by two structural changes from the round-4 playtest — no new action rule is added:

1. **Route starting zombies (D1):** Tutorial-01 now places 2–3 starting zombies along the path between the spawn zone and the exit zone, on pre-placed tiles. Surfacing the exit early is useless if characters cannot path through these zombies — players must either clear a path or find alternate routing, making the spatial+threat puzzle central and directly countering the draw-fast race strategy.
...
- Ship one v2 level (the tutorial) — level loader and SetupOptions dropdown remain wired; more levels are a follow-up. Tutorial-01 level design reworked in D1 (sparser tiles, Straight horde-origin tiles, ≥2 exit approaches, route-blocking starting zombies).
...
- Author the tutorial level first — it is the fallback target. Tutorial level must define `spawnZoneCells` (ordered, count >= 6), `exitZoneCells`, and `hordeOriginCells`. `exitZoneCells` must be on the opposite side from `spawnZoneCells`. No pre-placed tiles in `spawnZoneCells` or `exitZoneCells`. All starting zombie positions must have pre-placed tiles. **Tutorial-01 level-design intent (D1 — round-4 rework):** (a) Use SPARSER pre-placed tiles and prefer NON-Cross types (Straight, Elbow, Tee) — all-Cross pre-placed tiles open all base edges so zombies are never contained and the D1 anti-turtle break-out almost never fires; (b) place STRAIGHT tiles (not Cross) at the horde-origin cells — a second zombie piling onto a Straight-tiled horde origin can become contained and trigger break-out, making the anti-turtle mechanic meaningful; (c) the exit zone must have ≥2 distinct rotation-aware approach cells (catalogue AC-v2-5 sub-criterion — prevents near-stall from a single-cell approach); (d) place 2–3 starting zombies on pre-placed tiles along the route between the spawn zone and the exit zone (NOT adjacent to spawn cells) so players must navigate or clear them — this makes the spatial+threat puzzle central and counters the draw-fast-to-exit strategy; starting zombie count of 2–3 is the design target, positioned to threaten the main path. All other AC-v2-5 invariants (zones disjoint, ApPoolSize[n] ≥ MinActionsPerTurn, rotation-aware solvability, etc.) apply.
...
- **D2 — Zombie-tile spacing:** Assert that in a freshly constructed deck for each player count, no two zombie tiles appear within `ZombieTileMinSpacing=2` positions of each other in the middle band (when the band is large enough to honour the constraint — all 6 counts satisfy this per AD-OB-12 worked examples). Assert that the WARNING log is emitted when the band is artificially forced too small to honour the gap (unit test with overridden constants).
...
**Revision 7 — round-4 playtest balance & level pass**
**Status:** Refined v7 — balance & level pass (round-4)
...
21. Every `hordeOriginCell` must have a pre-placed level tile (catalogue validation — F3). Tutorial-01 horde-origin tiles must be Straight type (D1).
