...
**Revision:** 6 — round-2 playtest convergence. Applies 8 fixes (F1–F8): qualifying-actions escape-hatch (F1), deck construction pre/post-deal clarity (F2), catalogue validation additions (F3), zombie iteration phase-ordering clarity (F4), forced-zombie-tile EndTurn rejection (F5), MF-1 atomic resolution counter (F6), balance constant updates (F7), and known-limitations addition (F8). AC count: 79 (up from 76). AD count: 14 (unchanged; AD-OB-4, AD-OB-5, AD-OB-6, AD-OB-7, AD-OB-9, AD-OB-12, AD-OB-13 updated in place).
...
| `StartingHandSize` | `int` | 2 | Non-zombie, non-exit tiles dealt at CreateInitialState (DD3). May be reduced to 1 if safe opening proves too generous — playtest-tuning item. Balance TBD. |
| `SafeOpeningFraction` | `double` | 0.30 | Top fraction of raw pool guaranteed zombie/exit-free (DD3, F2). May be reduced to 0.20 if safe opening proves too generous — playtest-tuning item. Balance TBD. |
| `ApPoolSize` | `int[]` per count | `[5, 4, 4, 3, 3, 3]` (indices 1–6) | AP granted per turn; higher for low counts (DD2, F7). Balance TBD by playtest. |
...
| `ExitBandFraction` | `double[]` per count | `[0.50, 0.35, 0.30, 0.28, 0.26, 0.25]` (indices 1–6) | Exit tile placed randomly in the last X fraction of the deck (post-deal); higher fraction = earlier exit for low counts (DD2, F7). Balance TBD by playtest. |
...
- [ ] **AC-v2-8b — DrawTile-as-last-AP atomic resolution (MF-1, F6):** When a `DrawTile` action spends the player's LAST action point (i.e., `actionPointsRemaining` would reach 0 after this draw) AND the drawn tile is a zombie tile (or any tile that would otherwise create a forced-placement obligation), the forced obligation is resolved ATOMICALLY within the same `Handle` invocation — there is NO state in which `actionPointsRemaining == 0` while a forced zombie tile is held. The server resolves the obligation immediately using the same logic as `PlaceZombieTile`/forced-discard: find the first valid in-grid tiled cell (numeric ordering: lowest q; for equal q, lowest r) among empty non-exit-zone in-grid tiled cells; if one exists, place the zombie tile there and spawn a zombie at that coord (co-location elimination check runs immediately per AC-v2-31c); if no such cell exists, append the zombie tile to `discardPile`. The atomic forced placement (or discard) increments `qualifyingActionsThisTurn` by 1 (it is a qualifying action equivalent to PlaceZombieTile) and does NOT decrement `actionPointsRemaining` below 0 (AP is already 0 at point of resolution). After this atomic resolution, the minimum-actions obligation is considered satisfied in all cases — no further check is performed. The turn ends normally: the seat is added to `seatsActedThisRound`; `activeSeat` → `null`; `actionPointsRemaining` → 0; `qualifyingActionsThisTurn` → 0. There is NO carry-over of a forced obligation to a later turn, to the next round boundary, or to any subsequent `Handle` call.
