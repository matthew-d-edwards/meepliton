...
**Status:** Refined v9 — final playtest tuning (turn-and-move chase, solo difficulty)
**Date:** 2026-06-19
...
Hex Escape (Outbreak) is played on a sparse hexagonal grid identical to v1. Players collectively draw tiles from a shared deck each round by spending action points (AP); they place and rotate tiles to create open paths and move their characters toward a hidden exit. Characters representing each player start on the first tile they place, which must fall within the level's spawn zone on one side of the board. The exit zone is on the opposite side and is reserved — no normal tile may be placed there. Zombie tokens start at fixed centre-seed cells near the middle of the board. Each round, every zombie may both rotate one adjacent pipe tile and step one tile toward the nearest survivor (turn-and-move chase). Ending a turn on a cell adjacent to a zombie is lethal even through a closed pipe — the zombie can turn that pipe and step through in the same round. Any character sharing a cell with a zombie is eliminated immediately. A single special exit tile is shuffled into the bottom portion of the deck (band width tunable per player count); when a player draws it, the server immediately places it deterministically on an exit-zone cell — the player does not choose where. Play ends when all placed, non-eliminated characters physically occupy the exit cell (win) or all characters who were ever placed are eliminated (loss). Zombie cards never enter a player's hand; when drawn, the server spawns a zombie from the nearest centre seed and shoves the horde line outward toward the nearest survivor.

Each player's turn is governed by an **action-point (AP) pool of `ApPoolSize` AP** (tunable per player count; see Tunable Constants). On their turn a player may spend AP in any order and any mix on four atomic actions: draw a tile (1 AP), place a tile from hand (1 AP), rotate a player-placed tile (1 AP), or move their character (1 AP). A `MoveCharacter` action slides the character the full clear length of the connected pipe network in one action — not just one hex. Zombies block the tunnel: the slide may not pass through or land on a zombie-occupied tile. A player must spend at least `MinActionsPerTurn` qualifying actions before they may end their turn. Unused AP do not carry over.

To prevent turtling (sealing zombies away), a zombie that is fully contained — no connection-rule-valid move in any direction — both rotates its own tile to break out and spawns an additional zombie on an adjacent in-grid tiled cell. New zombies otherwise enter the board only when a zombie card is drawn (see zombie-card spawning above). There is no automatic per-round horde spawn from fixed origin cells; `hordeOriginCells` is empty for all authored levels.
...
| `HandSize` | `int` | 5 | Maximum tiles in hand. |
| `MaxMoveDistance` | `int` | 1 | Retained as a constant but not used as a movement limit — `MoveCharacter` now slides the full connected pipe length. Present for backward-compatibility with tests that reference it. |
| `MaxZombies` | `int` | 200 | JSONB growth safety cap. Structurally fixed for v2. |
| `StartingHandSize` | `int` | 3 | Non-zombie, non-exit tiles dealt at CreateInitialState (DD3). Balance TBD by playtest. |
...
| `ExitBandFraction` | `double[]` per count | `[0.40, 0.40, 0.40, 0.35, 0.32, 0.30]` (indices 1–6) | Exit tile placed randomly in the last X fraction of the deck (post-deal); higher fraction = earlier exit for low counts (DD2, F7, D2). Solo reduced from 0.50→0.40 so the exit sits deeper into the deck — the player digs further, surfacing more zombie cards during the hunt. Raised mid counts (2p: 0.35→0.40; 3p: 0.30→0.40; 4p: 0.28→0.35; 5p: 0.26→0.32; 6p: 0.25→0.30) so the exit surfaces earlier — a 3-player game was unwinnable because the team was overrun before the exit appeared. Balance TBD by playtest. |
| `ZombieTileMinSpacing` | `int` | 1 | Minimum deck-position gap between any two zombie tiles in the middle band during construction (D2, AC-v2-1b, D3/v8). Prevents an uncontrollable difficulty cliff from clustered zombie draws. Changed from 2→1 in v8 D3: with the corrected gate formula (minSlotsRequired = z+(z-1)×(spacing+1)), spacing=1 is satisfiable for all six player counts — the fallback uniform-random distribution is a rare safety net, not expected in normal play. spacing=2 was unsatisfiable for 3p–6p under the corrected gate. Balance TBD. |
...
- [ ] **AC-v2-1b — Deck construction with safe opening and exit band (DD3, DD2, F2):** The deck is constructed using two distinct quantities to avoid ambiguity between pre-deal and post-deal sizes:
  ...
  **Invariant after dealing:** the top `(safeCount − StartingHandSize × playerCount)` entries of `deck[]` are guaranteed to contain no zombie or exit tiles. Zombie tiles in the middle band are separated by at least `ZombieTileMinSpacing` positions from each other — with `ZombieTileMinSpacing=1` and the corrected gate formula, spacing is honoured for all six player counts (the fallback is a rare safety net, not expected in normal play; see AD-OB-12). Deck composition totals (post-deal) match the per-count scaling table (AD-OB-12).
...
- [ ] **AC-v2-17 — Exit zone is reserved:** _(The `PlaceZombieTile` clause below is superseded — players never place zombie tiles since the v2 zombie-growth rework; the `PlaceTile` exit-zone rule is unchanged.)_ `PlaceTile` and `PlaceZombieTile` targeting a cell in `exitZoneCells` are rejected with `"Cannot place tiles in the exit zone."`. This applies to all players and all tile types.
...
- [ ] ~~**AC-v2-23 — PlaceZombieTile valid:**~~ _**SUPERSEDED** — v2 zombie-growth rework: zombie tiles never enter a player's hand; the horde grows from drawn zombie cards via `SpawnHordeAtCentre`. The `PlaceZombieTile` handler was removed in the dead-code cleanup PR (the wire action is retained but rejected). Criterion preserved for record:_ Given `activeSeat` holds a zombie tile and dispatches `PlaceZombieTile { coord }`, the coord is an in-grid cell WITH A PLACED TILE (not tile-less), NOT in `exitZoneCells`, and NOT currently zombie-occupied (C4 unified spawn rule), and the forced zombie-tile obligation is active (player must place zombie tile before other non-forced actions), then: the cell is populated with the zombie tile; the tile is removed from hand; a new zombie token is spawned at coord with a stable generated id; co-location elimination check runs (if any character is at coord, eliminate it); `actionPointsRemaining` decremented by 1; `qualifyingActionsThisTurn` incremented by 1 (PlaceZombieTile is a qualifying action). Win check runs immediately (if `exitRevealed`). Note: the zombie tile has a normal `tileType` drawn from the standard set, placed at rotation 0, fixed (non-rotatable by players — but rotatable by D1 break-out, see AD-OB-5). The zombie tile placement obligation takes precedence over all other actions on that player's turn and counts toward the minimum qualifying-actions requirement.

- [ ] ~~**AC-v2-24 — PlaceZombieTile on occupied cell rejected:**~~ _**SUPERSEDED** — v2 zombie-growth rework: the `PlaceZombieTile` handler was removed; players never place zombie tiles. Criterion preserved for record:_ Rejected with `"Cell is already occupied."`. State unchanged. Obligation remains.

- [ ] ~~**AC-v2-25 — PlaceZombieTile with no legal in-grid tiled cell:**~~ _**SUPERSEDED** — v2 zombie-growth rework: the `PlaceZombieTile` handler and `DoForcedZombieDiscard` were removed; zombie tiles never enter the hand. Criterion preserved for record:_ Given a player holds a zombie tile but every in-grid tiled non-exit-zone cell is either zombie-occupied or otherwise unavailable, the zombie tile is appended to `discardPile` (no spawn); `actionPointsRemaining` decremented by 1; `qualifyingActionsThisTurn` incremented by 1 (PlaceZombieTile/forced-discard is a qualifying action). The forced discard costs 1 AP. Win check runs immediately (if `exitRevealed`).
...
- [ ] **AC-v2-32d — Phase 3 — turn-and-move chase (F4, D5):** Phase 3 iterates ALL zombies alive at the START of Phase 3, in stable id order. This includes zombies spawned during Phase 2 (break-out spawns) — they are alive at the start of Phase 3 and therefore act this round. (Phase 2's frozen-id-list applies only to Phase 2 containment evaluation; it does not exclude Phase-2-spawned zombies from Phase 3.)

  For each zombie in this list, perform **both** sub-steps in order:

  **(a) Turn a pipe:** the zombie selects the nearest survivor (shortest path by tile count; ties broken by lowest direction index). If an adjacent tile can be rotated to open an edge toward that survivor — including the tile on the cell between the zombie and an adjacent survivor — the server rotates it. Only player-placed, non-fixed tiles may be rotated; pre-placed level tiles and the exit tile are immune. This rotation can create a live connection between the zombie and a survivor **in the same round**, meaning a survivor ending a turn adjacent to a zombie is at risk even through a currently closed pipe.

  **(b) Step one tile:** after the turn sub-step, check the full connection rule AND exit-zone exclusion in the direction of the nearest survivor: the move is valid if the zombie's tile has an open edge in that direction AND the neighbour exists in the grid AND the neighbour's tile has an open edge in the opposite direction AND the neighbour cell is NOT in `exitZoneCells`. If valid, move the zombie there; otherwise the zombie stays (but the pipe turn from sub-step (a) sets up an approach for next round). **No zombie may ever move into an exit-zone cell** (D5 — code enforces this as a hard guard after the connection rule check).

  A zombie that is fully isolated (no in-grid tile neighbours) skips both sub-steps and holds.

  Store `{ zombieId, pipeTurned, direction, moved }` in `lastZombieRolls`. Co-location elimination check runs after each zombie step (AC-v2-31b). Phase 3 reads the LIVE post-Phase-2 grid — break-out rotations applied in Phase 2 ARE visible to Phase 3 (SF-5 two-snapshot rule: Phase 2 containment uses frozen pre-phase snapshot; Phase 3 reads live post-Phase-2 state).

  **Tester assertion note (D5):** a unit test must assert that a zombie whose chase direction points to an exit-zone neighbour stays in place and `moved: false` is recorded, even when the connection rule would otherwise permit the move.
...
- [ ] ~~**AC-v2-49 — PlaceZombieTile on tile-less cell rejected:**~~ _**SUPERSEDED** — v2 zombie-growth rework: the `PlaceZombieTile` handler was removed; players never place zombie tiles. Criterion preserved for record:_ PlaceZombieTile targeting an in-grid cell that has no placed tile is rejected with `"Cannot place zombie on a cell without a tile."` (C4 — all zombie spawn sources require a tiled target cell).
...
- [ ] **AC-v2-52 — Phase-3 zombie iteration includes Phase-2 spawns (F4):** A unit test crafts a state with a contained zombie that breaks out and spawns a new zombie during Phase 2. Assert that Phase 3 iterates both the original zombie AND the newly spawned zombie (i.e., `lastZombieRolls` contains entries for both). This confirms that Phase 2's frozen-id-list applies only within Phase 2 and that Phase-2-spawned zombies perform their turn-and-move chase in Phase 3.
...
All randomness (deck shuffle at start including exit-tile placement) uses `Random.Shared` called inside `Apply`/`Handle`, matching the SushiGo/LoveLetter precedent. Zombie chase movement is deterministic — no per-zombie die roll. The zombie's action (which pipe it turned, whether it stepped, target direction) is stored in state as `lastZombieRolls: [{ zombieId, pipeTurned, direction, moved }]` for animation and audit. No RNG seed is stored in state. No change to `GameContext`.
...
AP actions (draw, place, rotate, move) are all deterministic given the stored deck order. Zombie chase is also fully deterministic — no RNG surface at the round boundary. The only RNG surface is (a) deck shuffle at `CreateInitialState` (stored order persists in `deck`). No new RNG surface is introduced by the AP economy, D1 (containment), or D2 (horde).
...
- `lastZombieRolls: [{ zombieId, pipeTurned, direction, moved }]` — reset each ZombieMovement run. `pipeTurned` records which tile the zombie rotated (null if none); `direction` is the direction chased; `moved` is true if the zombie stepped.
...
- ~~`hordeOriginCells: string[]`~~ — **REMOVED** in the dead-code cleanup PR. No authored level ever populated it, so the fixed-origin horde subsystem (`hordeOriginCells`, `RunPhase4HordeSpawn`, `HordeRatePerRound`, `HordeStartRound`) was fully inert and was deleted. Zombie growth comes solely from zombie-card draws at the centre seed cells (`SpawnHordeAtCentre`).
...
Containment is checked per-zombie in stable id order before the Phase-3 chase. Zombies that are merely unable to step toward a survivor on a given round (normal stall) are NOT contained — full containment requires every direction to be blocked.
...
**Two distinct snapshot rules by phase (SF-5):** Phase 2 containment uses the frozen pre-phase snapshot (above). Phase 3 turn-and-move chase reads the LIVE post-Phase-2 grid — break-out rotations applied in Phase 2 ARE visible to Phase 3. This is intentional: break-out frees the zombie this round.
...
**(b) Zombie-card spawning (replaces per-round horde):** New zombies enter play when a zombie card is drawn during the Actions phase. The server spawns from a centre-seed cell and shoves the existing horde line outward; auto-drawn tiles may chain further spawns. There is no per-round fixed horde spawn from `hordeOriginCells` — that field is empty for all authored levels. `HordeRatePerRound` and the D2b horde-spawn path remain in code as a deferred hook but are inactive (`hordeOriginCells` is always empty).
...
- Phase-3 chase movement — step sub-steps whose target is an exit-zone cell are skipped entirely (AC-v2-32d, D5). This is a hard invariant: the connection rule alone does not prevent movement into the exit zone, so the exit-zone check is applied as a separate guard AFTER the connection rule check.

**Tester assertion (D5):** A unit test must assert that a zombie chasing toward an exit-zone cell stays in place and `moved: false` is recorded, even when the connection rule would otherwise permit the move.
...
**Band math worked examples (D2, D3, v8 — verified with StartingHandSize=1, SafeOpeningFraction=0.20, updated ExitBandFraction, corrected spacing gate, and ZombieTileMinSpacing=1):**

For each player count n, verify: (a) safeRemaining ≥ 0, (b) middleBand ≥ zombieTileCount, (c) exitBand ≥ 1, (d) safeRemaining + middleBand + exitBand = postDealSize, (e) no negative or overlapping bands, (f) **middleBand ≥ minSlotsRequired (corrected)** where `minSlotsRequired = zombieCount + (zombieCount − 1) × (ZombieTileMinSpacing + 1)` = `zombieCount + (zombieCount − 1) × 2` for `ZombieTileMinSpacing=1`. This corrected gate matches the placement loop's actual step of `ZombieTileMinSpacing + 1 = 2` positions per zombie advance. With `ZombieTileMinSpacing=1`, all six player counts satisfy constraint (f) — spacing is honoured for all counts.

| Players | postDealSize | rawPoolSize | safeCount | safeRemaining | exitBandFrac | exitBandStart | exitBand | middleBand | zombieTiles | minSlotsRequired (corrected) = n+(n−1)×2 | safeRemaining ≥ 0? | middleBand ≥ zombieTiles? | middleBand ≥ minSlotsRequired? |
|---------|-------------|-------------|-----------|---------------|--------------|---------------|----------|------------|-------------|------------------------------------------|-------------------|--------------------------|-------------------------------|
| 1       | 30          | 31          | 6         | 5             | 0.40         | 18            | 12       | 13         | 6           | 6+(5×2)=16                               | 5 ≥ 0 ✓           | 13 ≥ 6 ✓                 | 13 < 16 ✗ (fallback)           |
| 2       | 40          | 42          | 8         | 6             | 0.40         | 24            | 16       | 18         | 5           | 5+(4×2)=13                               | 6 ≥ 0 ✓           | 18 ≥ 5 ✓                 | 18 ≥ 13 ✓                     |
| 3       | 50          | 53          | 10        | 7             | 0.40         | 30            | 20       | 23         | 7           | 7+(6×2)=19                               | 7 ≥ 0 ✓           | 23 ≥ 7 ✓                 | 23 ≥ 19 ✓                     |
| 4       | 60          | 64          | 12        | 8             | 0.35         | 39            | 21       | 31         | 10          | 10+(9×2)=28                              | 8 ≥ 0 ✓           | 31 ≥ 10 ✓                | 31 ≥ 28 ✓                     |
| 5       | 70          | 75          | 15        | 10            | 0.32         | 48            | 22       | 38         | 12          | 12+(11×2)=34                             | 10 ≥ 0 ✓          | 38 ≥ 12 ✓                | 38 ≥ 34 ✓                     |
| 6       | 80          | 86          | 17        | 11            | 0.30         | 56            | 24       | 45         | 15          | 15+(14×2)=43                             | 11 ≥ 0 ✓          | 45 ≥ 15 ✓                | 45 ≥ 43 ✓                     |

**Note on spacing-gate results (D3, v8, final playtest):** With the final solo zombie count of 6, only 2p satisfies constraint (f) under the corrected gate formula with `ZombieTileMinSpacing=1` (middle band 18 ≥ minSlotsRequired 13). 1p, 3p, 4p, 5p, and 6p all fail the gate and fall back to uniform random spacing, emitting the WARNING log. All six counts pass constraints (a)–(e). The tester spacing-assertion must confirm: 2p enforces spacing; 1p and 3p–6p emit the WARNING.

**Balance notes (D2, D3, v8, final playtest):** `ApPoolSize[1] = 5` is higher than for 2–6 players (all now 4) — solo gets 1 extra AP. `ExitBandFraction[1]` reduced from 0.50 to 0.40 (solo exit sits deeper so the player surfaces more zombie cards during the hunt — harder solo). `ZombieTileCount[1]` raised to 6 (solo deck carries more zombie cards than 2p; solo horde maxes at 8 = 2 seeds + 6 cards). `ExitBandFraction` raised for 2–6p so the exit surfaces earlier across all multi-player counts; the critical fix is 3p (0.30→0.40), which was unwinnable because teams were overrun before the exit appeared. `ZombieTileMinSpacing` reduced from 2 to 1 in v8 D3; the corrected gate formula with 6 solo zombie tiles means 1p also falls back to uniform random spacing (middle band 13 < minSlotsRequired 16) — WARNING fires for 1p, 3p–6p; only 2p guarantees spacing. All values are balance TBD by playtest; solo is intentionally harder (playtested as challenging but winnable — a skilled player loses some, wins some).
...
- Full-pipe slide movement — one `MoveCharacter` action slides the character the full length of the connected pipe in one action (not one hex). `MaxMoveDistance` constant is retained but not enforced as a one-hex limit. Zombies block the tunnel.
- Zombie cards never enter the player's hand — they trigger a server-side spawn from the centre seeds. Players never place zombie tiles by hand.
- Fixed hand cap 5 — constant `HandSize = 5`.
...
- Discard reshuffle
- Player-held zombie tiles or player-chosen zombie placement (zombie cards are handled server-side)
...
- Round-boundary sequence (all in one `Handle` invocation): ZombieMovement transition → D1 containment evaluation (Phase 2, frozen snapshot) → turn-and-move chase (Phase 3, live post-Phase-2 grid) → D2b horde spawn → loss check → round advance.
...
- **C4 — Tiled-cell spawn rule (unified):** ALL zombie spawn sources (zombie-card server spawn, D1 break-out spawn) target only IN-GRID cells WITH A PLACED TILE, not zombie-occupied, not in exitZoneCells. Catalogue validation asserts every starting zombie position has a pre-placed tile. Defensive rule: a zombie on a cell with no tile cannot move and is treated as not-contained (no break-out); validation makes this unreachable.
- ~~D2b horde (deferred / inactive)~~ — **REMOVED** in the dead-code cleanup PR. No authored level populated `hordeOriginCells`, so `RunPhase4HordeSpawn` was a guaranteed no-op; it, its `RunRoundBoundary` call site, `hordeOriginCells`, `HordeRatePerRound`, and `HordeStartRound` were deleted. The zombie-card spawn (`SpawnHordeAtCentre`) is the sole growth source, with the `MaxZombies` cap still applied there.
...
- MinActionsPerTurn enforcement: track `qualifyingActionsThisTurn` in state (counts DrawTile, PlaceTile, MoveCharacter only — RotateTile does not count; PlaceZombieTile is no longer reachable because zombie cards do not enter the player's hand). On `EndTurn`: check `qualifyingActionsThisTurn >= min(MinActionsPerTurn, numberOfQualifyingActionsAvailableThisTurn)` (AC-v2-10, F1). Short-circuit: if deck non-empty OR hand non-full, DrawTile is available → minimum is at least 1, skip further enumeration. Reject if minimum not met. (F1 implementation guidance.)
...
- All tunable constants in one `HexEscapeConstants` class: `MinActionsPerTurn`, `HandSize`, `MaxMoveDistance`, `MaxZombies`, `StartingHandSize`, `SafeOpeningFraction`, `ApPoolSize[]`, `ExitBandFraction[]`, `ZombieTileMinSpacing`, deck composition table.
...
- When the round boundary fires (all seats done), show a brief auto-dismissing beat before the zombie-movement overlay: **"Your turn is over — the horde moves"** with a one-line summary driven by `lastZombieRolls` (e.g. "3 zombies advanced · 2 turned a pipe to chase" or "The horde held its ground"). The beat dismisses automatically — no user action needed.
- Animate `lastZombieRolls` — show which pipe each zombie turned and whether it stepped forward.
...
- **MF-3 — Exit-zone never receives zombie (D5):** Assert D1 spawn and D2b horde never place zombie on exit-zone cell even when exit-zone cells are lowest-index candidates. Assert Phase-3 chase movement never moves a zombie to an exit-zone cell even when the connection rule would permit the move — the zombie stays in place and `moved: false` is recorded (D5, AC-v2-32d).
...
- **D2/D3 — Zombie-tile spacing and corrected gate:** The corrected construction gate uses `minSlotsRequired = zombieCount + (zombieCount − 1) × (ZombieTileMinSpacing + 1)`. With current constants (`ZombieTileMinSpacing=1` and final zombie counts), only 2p (middleBand=18, minSlotsRequired=13) passes the corrected gate. 1p (middleBand=13, minSlotsRequired=16), 3p, 4p, 5p, and 6p all fail and fall back to uniform random spacing with a WARNING. Assert: (a) for 2p, zombie tiles in a freshly constructed deck are separated by at least `ZombieTileMinSpacing=1` positions in the middle band; (b) for 1p and 3p through 6p, the WARNING log `"HexEscape: zombie tile spacing constraint could not be satisfied for {playerCount}p — band too small"` is emitted at deck construction; (c) a unit test with overridden constants that force `bandWidth < minSlotsRequired` (using the corrected gate formula) also triggers the WARNING; (d) a unit test using the OLD (incorrect) gate formula `zombieCount + (zombieCount-1) × ZombieTileMinSpacing` would NOT trigger the WARNING — assert the new code uses the corrected formula (verify by inspection or by confirming the WARNING fires where expected).
...
| v9 | 2026-06-19 | Final playtest tuning: zombie chase model → turn-and-move (each zombie may rotate one pipe AND step one tile per round, attack-adjacent rule); end-of-turn "horde moves" beat added; solo tuned (`ZombieTileCount[1]`→6, `ExitBandFraction[1]`→0.40); band math updated (1p now also hits spacing fallback); AC-v2-32d rewritten for turn-and-move |
...
- **Zombie-on-exit bug (D5):** Phase-3 movement was not excluding exit-zone cells as step targets. Code now blocks these steps. AC-v2-32d and AD-OB-7 updated.
...
- ~~**HordeOriginCell with no pre-placed tile (F3):**~~ **SUPERSEDED** (v2 zombie-growth rework). `hordeOriginCells` and the fixed-origin horde subsystem were removed as inert; the catalogue validation that asserted every `hordeOriginCell` has a pre-placed tile was removed with it (the starting-zombie-position assertion remains). _Original resolution retained for record: Catalogue validation asserted every `hordeOriginCell` had a pre-placed level tile, mirroring the starting-zombie-position assertion._
...
| ~~H7~~ | ~~Horde origin cells could overlap spawn zone~~ | **SUPERSEDED** (v2 zombie-growth rework): `hordeOriginCells` and the fixed-origin horde subsystem were removed as inert, so this invariant no longer applies and its catalogue test was removed. _Original resolution: AC-v2-5 catalogue asserted `hordeOriginCells ∩ spawnZoneCells = ∅`._ |
...
1. D1 containment check must run BEFORE Phase-3 chase.
...
3. Phase 3 turn-and-move chase reads the LIVE post-Phase-2 grid — break-out rotations from Phase 2 ARE visible to Phase 3 (SF-5).
...
9. Round-boundary sequence: transition → containment evaluation (frozen snapshot) → turn-and-move chase (live grid) → horde spawn → loss check → round advance (all in one Handle call).
...
19. Phase 3 zombie iteration includes Phase-2 break-out spawns — they perform their turn-and-move chase in Phase 3 (F4).
...
23. Phase-3 chase movement skips any step whose target is an exit-zone cell — even if the connection rule would otherwise permit the move. No zombie may ever occupy an exit-zone cell by spawn OR movement (D5, AC-v2-32d, AD-OB-7).
24. Zombie tiles in the middle band must be spaced with a minimum gap of `ZombieTileMinSpacing=1` deck positions where the band allows it; distribution is interval-based with small random jitter. Fall back to uniform random if the band is too small (most player counts), and emit a WARNING (D2, AC-v2-1b).
