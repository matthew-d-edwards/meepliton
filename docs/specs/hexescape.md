...
- `hordeOriginCells: string[]` — always empty for authored levels. Zombie growth comes from zombie-card draws at centre seed cells, not from fixed per-round origin cells. The field and the D2b horde-spawn code path are retained as a deferred extensibility hook.
...
- D2b horde (deferred / inactive): `hordeOriginCells` is empty for all authored levels; the D2b code path is never exercised. `HordeRatePerRound` and the horde loop are retained as an extensibility hook. MaxZombies cap applies if the path is ever activated.
...
- All tunable constants in one `HexEscapeConstants` class: `MinActionsPerTurn`, `HandSize`, `MaxMoveDistance`, `MaxZombies`, `StartingHandSize`, `SafeOpeningFraction`, `ApPoolSize[]`, `HordeRatePerRound[]` (deferred — inactive while `hordeOriginCells` is empty), `ExitBandFraction[]`, `ZombieTileMinSpacing`, deck composition table.
