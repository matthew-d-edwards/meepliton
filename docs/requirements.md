...
**Version 0.7 | Living Document | 2026-06-16**
...
- [x] Fifth game module — **Hex Escape (Outbreak)** v2 (co-operative hex-pipe puzzle, 1–6 players; first co-op title on the platform; players place and rotate pipe tiles, slide characters the full length of connected pipes in one action, and must reach a hidden exit tile before zombie tokens — which grow from centre-seed Cross tiles when zombie cards are drawn, use turn-and-move chase each round (each zombie may rotate one adjacent pipe AND step one tile toward the nearest survivor, so a survivor ending a turn adjacent to a zombie is at risk even through a closed pipe), and eliminate characters on contact — overwhelm the team; v1's threat-counter design is superseded; v2 ships one authored level with the level-picker wired for follow-up levels; validates `GameOverEffect(winnerId: null)`, `MinPlayers = 1`, `SetupOptions` level picker, per-player state projection via `ProjectStateForPlayer`, and constructor-injected `ILogger<T>` — see ADR-010, ADR-012, and ADR-013)
