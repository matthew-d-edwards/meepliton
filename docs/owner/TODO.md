...
- [ ] **2026-06-16** Hex Escape — spawn zone colour-only distinction: cells in `spawnZoneCells` (other than the player's own reserved spawn) are distinguished from empty cells only by a green stroke/tint (`--status-success`). A player's own reserved spawn cell gets a gold glow and "S" letter marker, but all other spawn cells have no text or icon cue. Colour-blind users (Deuteranopia) cannot distinguish the green spawn zone from the empty hex grid. Ask `ux` to add a short text label or pattern fill to all spawn zone cells. This is a visual redesign decision that requires `ux` input. (ally)
...
- [ ] **2026-06-16** Hex Escape — SVG cell board keyboard `aria-disabled`: when `canInteract=true`, ALL hex cells become `role="button"` in the tab order including exit-zone cells (which silently reject clicks), fixed pre-placed tiles (which silently do nothing), and cells in zombie obligation mode that have no tile. A keyboard-only user will Tab to these "buttons" and activate them with no feedback. Ask `frontend` to accept a `disabledCoords: Set<string>` prop from `Game.tsx` and apply `aria-disabled="true"` to non-actionable cells. This is a moderate complexity change — flag to `frontend` after the current story closes. (ally)
