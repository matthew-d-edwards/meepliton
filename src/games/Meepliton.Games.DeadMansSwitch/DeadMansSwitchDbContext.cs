...
public class DeadMansSwitchDbContext : DbContext, IGameDbContext
{
    private readonly IConfiguration? _configuration;

    // Used by DI (Scrutor) — IConfiguration is always available in the container.
    public DeadMansSwitchDbContext(IConfiguration configuration) => _configuration = configuration;

    // Used by IDesignTimeDbContextFactory for dotnet ef CLI tooling.
    internal DeadMansSwitchDbContext(DbContextOptions<DeadMansSwitchDbContext> options) : base(options) { }
