...
public class SushiGoDbContext : DbContext, IGameDbContext
{
    private readonly IConfiguration? _configuration;

    // Used by DI (Scrutor) — IConfiguration is always available in the container.
    public SushiGoDbContext(IConfiguration configuration) => _configuration = configuration;

    // Used by IDesignTimeDbContextFactory for dotnet ef CLI tooling.
    internal SushiGoDbContext(DbContextOptions<SushiGoDbContext> options) : base(options) { }
