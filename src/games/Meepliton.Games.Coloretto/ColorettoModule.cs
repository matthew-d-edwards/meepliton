...
public class ColorettoModule : IGameModule, IGameHandler
{
    ...
    {
        // Start with a mutable copy of the collection (only active colours + Joker)
        ...
        // taking into account which colours will end up in the top 3
        for (int j = 0; j < jokerCount; j++)
        {
            ...
            foreach (var color in activeColors)
            {
                ...
                // Simulate what the top-3 would be after adding joker here
                // We use the gain as a heuristic (greedy) — sufficient for this game
